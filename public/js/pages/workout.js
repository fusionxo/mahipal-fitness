...
let state = {
    ...
    templates: [],
    profile: null,
    charts: { volume: null, intensity: null, muscle: null },
...
const api = {
    getWorkouts: async () => {
        const res = await fetch('/api/workouts', { headers: { 'Authorization': `Bearer ${AppState.token}` } });
        if (!res.ok) throw new Error('Could not fetch workout data.');
        return res.json();
    },
    saveWorkout: async (workoutData) => {
        const hasId = !!workoutData._id;
        const url = hasId ? `/api/workouts/${workoutData._id}` : '/api/workouts';
